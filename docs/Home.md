...
This section provides detailed information about how to calculate **classification metrics** such as precision, recall, F1-score, F-beta scores, and more. The classification metrics are essential for evaluating the performance of classification models by comparing the predicted results with the ground truth.
...
Aggregation allows you to compute an overall metric from multiple classification tasks. This can be useful when you want to combine results from several tasks to get a single evaluation score.
...
### 3.2 Usage via REST API

The project offers a REST API for calculating metrics. You can send HTTP requests to the API to compute classification metrics as well as aggregate results across tasks. Swagger documentation is provided for easy testing and interaction.
